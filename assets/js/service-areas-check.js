(function initServiceAreaCheck() {
  ...
  function resolveMatch(raw) {
    if (typeof window.resolveServiceAreaFromText !== "function") return null;
    const meta = window.resolveServiceAreaFromText(raw);
    if (!meta) return null;
    if (!meta.matched) return null;
    return { area: meta, type: "resolved" };
  }
  ...
  function renderResult(match, raw) {
    ...
    if (!match) {
      const outside = typeof window.resolveServiceAreaFromText === "function"
        ? window.resolveServiceAreaFromText(raw)
